...
namespace LX
{
	...
	bool LXMysql::Query(const char *sql, unsigned long sqllen)
	{
		...
		//传入的是一个负数
		if (sqllen <= 0)
			sqllen = (unsigned long)strlen(sql); //做一个强制转换
		if (sqllen <= 0)
		{
	...
	std::vector<LXData> LXMysql::FetchRow()
	{
		...
		MYSQL_ROW row = mysql_fetch_row(result); // 一行
		if (!row)
		{
			return re;
		}
		...
		for (int i = 0; i < num; i++)
		{
			...
			data.size = lens[i];
			//获取列的类型
			auto field = mysql_fetch_field_direct(result, i); // 获取类型
			data.type = (FIELD_TYPE)field->type;			  // 枚举类型的强制类型转换
			re.push_back(data);
		}
		return re;
	}
	...
	//生成insert sql语句
	std::string LXMysql::GetInsertSql(XDATA kv, std::string table) // 可以用引用
	{
		string sql = "";
		...
		for (auto ptr = kv.begin(); ptr != kv.end(); ptr++)
		{
			...
			if (ptr->first[0] == '@')
			{
				vals += ptr->second.data; //它是一个函数，在mysql执行的函数
			}
			else
			{
				vals += "'";
	...
	bool LXMysql::InsertBin(XDATA kv, std::string table)
	{
		...
		for (auto ptr = kv.begin(); ptr != kv.end(); ptr++)
		{
			...
			bind[i].buffer_type = (enum_field_types)ptr->second.type; // 类型,强制转换一下
			i++;
		}
		//去除多余的逗号
		keys[keys.size() - 1] = ' ';
		...
		{
			mysql_stmt_close(stmt);
			cerr << "mysql_stmt_bind_param failed!" << mysql_stmt_error(stmt) << endl; // 不同的错误接口
			return false;
		}
		if (mysql_stmt_execute(stmt) != 0)
		{
			mysql_stmt_close(stmt); // 可以用异常，也可以用goto
	...
	int LXMysql::Update(XDATA kv, std::string table, std::string where)
	{
		...
		return mysql_affected_rows(mysql); // 返回影响的行数
	}
