...
char _getch()
{
	...
	termios old_tm;
	int fd = 0;						//文件描述符
	if (tcgetattr(fd, &old_tm) < 0) //获取
		return -1;

	//更改为原始模式，没有回显
	cfmakeraw(&new_tm);
	if (tcsetattr(fd, TCSANOW, &new_tm) < 0) //设置新的格式
	{
		return -1;
	}
	char c = getchar();
	if (tcsetattr(fd, TCSANOW, &old_tm) < 0) //设置回旧的格式
	{
		return -1;
	}
	return c;
...
{
	//统计总数量
	vector<std::string> vec_str = cmds;
...
void XClient::c_search(std::vector<std::string>cmds)
{
	...
	auto rows = my->GetResult(sql.c_str());
	// 无索引: time  sec =0.003104 sec
	// 有索引: time  sec =0.000949 sec
...
void XClient::c_log(std::vector<std::string>cmds)
{
	...
	cout << "Page = " << page << " PageCount = " << pagecount << endl;//显示
}
